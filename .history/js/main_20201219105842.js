window.app = new Vue({
  ...
    this.params = eval(decodeURIComponent(a1.substring(a1.indexOf('params=')+4)));
    ...
    //this.joinClassroom()  //默认进入1065865905房间号;
  ...
  methods: {
	...
	async queryStudent(id){		//根据学生id查学生信息
		let students=[];
  ...
  //      url:"https://ppt2h5-1400449991.qcloudtiw.com/0vg7cti49vgr2jl55pub/index.html"
    ...
    // 登录
    login() {
    	console.log("userId=",this.userId,"userSig===>",this.userSig,"userId==>");
      this.tic.login({
        userId: this.userId,
    ...
    joinClassroom() {
      ...
      this.recordId =""+ new Date().getTime() + this.roomID + this.params.teacherId;
      if (!this.roomID) {
        this.showErrorTip('房间号不能为空');
        return;
      }
    ...
    sendCustomGroupMsg() {
      ...
      if (this.imMsg.common.toUser) { // C2C 自定义
        this.tic.sendCustomMessage(_this.imMsg.common.toUser, _this.imMsg.custom.data, function (res) {
